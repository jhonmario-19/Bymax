...
class UserListController {
  ...
  Future<bool> verifyAdminAccess() async {
    try {
      ...
      final isAdmin = userDoc.exists && (userDoc.data()?['isAdmin'] == true || userDoc.data()?['rol'] == UserController.ROLE_ADMIN);
  ...
  Future<Map<String, dynamic>> loadUsersAndFamilies() async {
    try {
      ...
      if (usersSnapshot.docs.isEmpty) {
        print("No se encontraron usuarios creados por este admin. Verificando si existen usuarios:");
        ...
        print("Total de usuarios en la colección: ${allUsersSnapshot.docs.length}");
      ...
      List<Map<String, dynamic>> usuarios = usersSnapshot.docs
          .map((doc) => {
                ...doc.data(),
                'id': doc.id,
              })
      ...
      final familiasSnapshot = await FirebaseFirestore.instance
          .collection('familias')
          .get();

      print("Cantidad de familias encontradas: ${familiasSnapshot.docs.length}");
